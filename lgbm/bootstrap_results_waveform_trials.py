...
def load_results(time):
    ...
    """
    with open(prefix_path + time + "_tachycardia_waveform_trial_results_auroc.json", 'r') as f:
        total_tachy = json.load(f)
    with open(prefix_path + time + "_tachycardia_waveform_trial_results_pr.json", 'r') as f:
        total_tachy_pr = json.load(f)
    with open(prefix_path + time + "_hypoxia_waveform_trial_results_auroc.json", 'r') as f:
        total_hypoxia = json.load(f)
    with open(prefix_path + time + "_hypoxia_waveform_trial_results_pr.json", 'r') as f:
        total_hypoxia_pr = json.load(f)
    with open(prefix_path + time + "_hypotension_waveform_trial_results_auroc.json", 'r') as f:
        total_hypotension = json.load(f)
    with open(prefix_path + time + "_hypotension_waveform_trial_results_pr.json", 'r') as f:
...
def bootstrap_results(hpams, data_paths, config_names, config_dict, time, task, num_bootstrap=10000):
    ...
    for name in config_names:
        ...
        if config["include_wave"] and config["wave_type"] == "Both":
            ...
            pleth_model = data_paths[task]["pleth_dir_path"] + str(config["wave_size"]) + data_paths[task]["model_suffix"]
            ecg_model = data_paths[task]["ecg_dir_path"] + str(config["wave_size"]) + data_paths[task]["model_suffix"]            
        ...
        final_pred = final_lgbm_class.predict_proba(xval)[:, 1]
    ...
    for i in range(num_bootstrap):
        bootstrap_indices = np.random.choice(range(len(yval)), size=len(yval), replace=True)
        for key in all_preds.keys():
            preds = [all_preds[key][i] for i in bootstrap_indices]
            labels = [yval[i] for i in bootstrap_indices]
...
def save_bootstrap(task, time, bs_auroc_dict, bs_prscore_dict):
    ...
    auroc_results.to_csv(prefix_path + "lgbm/" + time + "_" + task + "_waveform_trial_auroc_bootstrap_testrun.csv", index=False)
    pr_results.to_csv(prefix_path + "lgbm/" + time + "_" + task + "_waveform_trial_prscore_bootstrap_testrun.csv", index=False)
...
def get_diff_tables(input_bootstrap_file1, time, task, score, combine = False):
    ...
    if not combine:
        results.to_csv(prefix_path + "lgbm/" + time + "_" + task + "_waveform_trial_" + score + "_TriageCCDiff.csv", index=False)
    else:
        results.to_csv(prefix_path + "lgbm/" + time + "_" + task + "_waveform_trial_" + score + "_TriageCCDiff_Combined.csv", index=False)
    return results
...
def get_output_tables_point_CI(input_bootstrap_file1, time, task, score="auroc"):
    ...
    results = pd.DataFrame.from_dict(total_results)
    results.to_csv(prefix_path + "lgbm/" + time + "_" + task + "_waveform_trial_" + score + "_CIandPoint.csv", index=False)
    return results
...
def get_output_tables_point_CI_diff(input_bootstrap_file1, time, task, score, percentiles = [2.5, 97.5]):
    ...
    results = pd.DataFrame.from_dict(total_results)
    results.to_csv(prefix_path + "lgbm/" + time + "_" + task + "_waveform_trial_" + score + "_CIandPoint_" + str(percentiles[1] - percentiles[0]) + "_" + "diff.csv", index=False)
    return results
...
def main():
    ...
    all_config_path = "/deep/group/ed-monitor-self-supervised/test_models_v1/ed-monitor-decompensation/waveform_trial_configs.json"
    with open(all_config_path, 'r') as f:
        config_dict = json.load(f)
    config_names = list(config_dict.keys())
    
    file_path_config = "/deep/group/ed-monitor-self-supervised/test_models_v1/ed-monitor-decompensation/waveform_trial_path_configs.json"
    ...
    for time in times:
        ...
        for task in tasks:
            ...
            for score in scores:
                bootstrap_prefix_path = prefix_path + "lgbm/" + time + "_" + task + "_waveform_trial_" + score
