...
def save_params_metrics_task(time, task, auroc, pr, params):
    with open(prefix_path + time + "_" + task + "_waveform_trial_results_params.json", 'w') as f:
        json.dump(params, f)
    with open(prefix_path + time + "_" + task + "_waveform_trial_results_auroc.json", 'w') as f:
        json.dump(auroc, f)
    with open(prefix_path + time + "_" + task + "_waveform_trial_results_pr.json", 'w') as f:
        json.dump(pr, f)
...
def test_light_gbm(data_tuple, hparams):
    ...
    auroc = roc_auc_score(yval, final_pred)
    prscore = average_precision_score(yval, final_pred)
