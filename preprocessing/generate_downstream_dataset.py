...
#             print(f"Continuing search...")
    ...
    for k, waveform_type in enumerate(WAVEFORM_COLUMNS):
        ...
        if len(waveform) > int(waveform_len_sec * TARGET_FREQ):
            waveform = waveform[int(waveform_len_sec * TARGET_FREQ) - len(waveform):]
        elif len(waveform) < int(waveform_len_sec * TARGET_FREQ):
            waveform = np.pad(waveform, (int(waveform_len_sec * TARGET_FREQ) - len(waveform), 0))
...
def process_patient(input_args):
    ...
    try:
        ...
        with h5py.File(filename, "r") as f:
            ...
            recommended_trim_end_sec = int(row["recommended_trim_end_sec"].item())
